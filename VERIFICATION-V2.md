...
1. [ ] Launch the app on a clean install (or after deleting and reinstalling).
       Open the routine list. Confirm seed routines load without crash.
2. [ ] Open a routine that includes Plank. The Plank row trailing label reads
       a duration range (e.g. `30–45s`), not a rep range.
3. [ ] Custom-exercise editor → create a new exercise with metric type
       "Time". Save. Confirm it lands in the exercise library.
4. [ ] Edit that brand-new custom exercise. The metric-type picker is
       editable (not locked). Switch to "Reps + Weight"; save; reopen; the
       change persists.
5. [ ] Start a session that includes Plank. SetRow shows a single "Seconds"
       field (no KG / Reps fields). Enter `35`; tap the checkmark; the set
       persists. Add a second set with `40`. Finish the session.
6. [ ] After a set has been logged against the Time custom exercise from step
...
7. [ ] Exercise history view for Plank: chart Y axis shows seconds (legend
       "TOP-SET SECS"). Session list rows show `{n}s`, not `× n`.

## Previous-execution recall (3 steps)

8. [ ] Complete one session of a lift routine. Return to that routine's
...
9. [ ] Open a routine that has never been completed. The card shows the
       empty state copy ("First time on this routine — no comparison yet").
10. [ ] Edit the just-completed routine: remove one exercise that was logged
...
11. [ ] Files app → On My iPhone → Hybrid. Three files visible:
        `Hybrid.sqlite`, `hybrid-latest.json`, `hybrid-schema.md`.
12. [ ] Open `hybrid-schema.md` from the Files app. Content includes
        `schema_version: 2.0`, the Envelope section, and the metric_type
        semantics table.
13. [ ] Note the mtime of `hybrid-latest.json`. Finish a new session. Re-open
        Files; mtime is more recent than before. JSON opens to an object
        with `schema_version: "2.0"` and `data: { ... }`.
14. [ ] Settings → toggle "Auto-update LLM snapshot" OFF. Finish another
        session. Re-check `hybrid-latest.json` mtime — unchanged.
15. [ ] Settings → "Refresh LLM snapshot now" button. mtime updates.
